...
library Clone {
    ...
    function _clone(address contractToClone) internal returns (address cloned) {
    ...
    function _clone2(address contractToClone, bytes32 salt) internal returns (address cloned) {
        bytes memory bytecode = contractToClone.code;
        uint256 bytecodeSize = bytecode.length;
        uint8 lowerByte = uint8(bytecodeSize);
        uint8 upperByte = uint8(bytecodeSize / 256);
    ...
    function _predictClonedAddress(
